...
All notable changes to Cloud Doctor will be documented in this file.
...
- Per-project config via `.claude/cloud-doctor.local.md` (YAML frontmatter, 10 fields)
- `/cloud-doctor:setup` slash command for first-run config
...
- `$CLAUDE_PLUGIN_DATA` env var is referenced in Anthropic docs but not confirmed in use by other plugins in the official marketplace as of release. Fallback to `~/.claude/plugin-data/cloud-doctor/` handles unset case.
